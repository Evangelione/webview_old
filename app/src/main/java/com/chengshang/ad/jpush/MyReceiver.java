...
public class MyReceiver extends BroadcastReceiver {
        ...
        try {
            ...
            if (JPushInterface.ACTION_REGISTRATION_ID.equals(intent.getAction())) {
                ...
                if (flagJson1.getAsString().equals("1")) {
                    ...
                    i.putExtra("url", flagJson.getAsString());
                    i.setFlags(Intent.FLAG_ACTIVITY_NEW_TASK | Intent.FLAG_ACTIVITY_CLEAR_TOP);
                    context.startActivity(i);
                }
    ...
    private boolean processCustomMessage(Context context, Bundle bundle) throws JSONException {
        ...
        JsonParser parser = new JsonParser();
        JsonElement jsonElement = parser.parse(alert);
        JsonObject jsonObject = jsonElement.getAsJsonObject();
        //此处判断提示语音
        JsonPrimitive flagJson2 = jsonObject.getAsJsonPrimitive("user_type");
        switch (flagJson2.getAsString()) {
            case "apppoint"://服务者
            {
                Log.i("====apppoint", "====");
...
//                JsonPrimitive flagJson2 = jsonObject.getAsJsonPrimitive("user_type");
        ...
        if (flagJson1.getAsString().equals("1")) {
            ...
            SpeechTool.getInstance(context,0).initSpeech(msg);
        } else {
            //环信
