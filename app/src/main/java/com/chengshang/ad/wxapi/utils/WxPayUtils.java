...
/**
 * APP名： InitialProject
 * 包名：com.chengshang.ad.wxapi.utils
 * 作者：jiang-pc
 * 版本：
 * 创建日期：2019/5/28
 * 描述：
...
public class WxPayUtils {
    ...
    public void toWXPayNotSign(Context context) {
        Runnable payRunnable = new Runnable() {  //这里注意要放在子线程
            ...
            public void run() {
                ...
                APP.api.sendReq(request);//发送调起微信的请求
            }
        };
    ...
    public void toWXPayAndSign(Context context, String appid, final String key) {
        APP.api = WXAPIFactory.createWXAPI(context, null); //初始化微信api
        APP.api.registerApp(appid); //注册appid  appid可以在开发平台获取
        Runnable payRunnable = new Runnable() {  //这里注意要放在子线程
            @Override
            public void run() {
                PayReq request = new PayReq(); //调起微信APP的对象
